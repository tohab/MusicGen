...
public class Chords extends Thread {
	...
	public static void major(int key, int rep, int tempo)
	...
	public static void minor(int key, int rep, int tempo)
			throws InterruptedException, MidiUnavailableException {
		for (int i = 0; i < rep; i++) {
			mc[5].noteOn(60 + key, 100);
			new Thread().sleep(tempo);
	...
	public static void firstinversionmaj(int key, int rep, int tempo)
			throws InterruptedException, MidiUnavailableException {
		for (int i = 0; i < rep; i++) {
			mc[5].noteOn(64 + key, 100);
			new Thread().sleep(tempo);
	...
	public static void secondinversionmaj(int key, int rep, int tempo)
			throws InterruptedException, MidiUnavailableException {
		for (int i = 0; i < rep; i++) {
			mc[5].noteOn(67 + key, 100);
			new Thread().sleep(tempo);
	...
	public static void secondinversionmin(int key, int rep, int tempo)
			throws InterruptedException, MidiUnavailableException {
		for (int i = 0; i < rep; i++) {
			mc[5].noteOn(67 + key, 100);
			new Thread().sleep(tempo);
	...
	public static void start(int key, int tone, int tempo)
			throws InterruptedException, MidiUnavailableException {
		if (tone == 1) {
			while (true) {
				if (Math.random() < .9) {
